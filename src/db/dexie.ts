    ...
    if (idsToDelete.length > 0) {
      ...
      if (firestoreDb) {
        ...
        for (const id of idsToDelete) {
          await safeDeleteDoc(doc(firestoreDb, 'student_sessions', String(id)));
        }
      }
    }
  } catch (err) {
...
export async function deleteOnlyDemoTestData() {
  ...
  // 8. Delete from Cloud Firestore asynchronously (safe from transaction-inactivation)
  if (firestoreDb) {
    try {
      const { doc } = await import('firebase/firestore');
      const deleteCloud = async (tableName: string, items: any[]) => {
        for (const item of items) {
          if (item.id) {
            await safeDeleteDoc(doc(firestoreDb, tableName, String(item.id)));
          }
        }
      };

      await deleteCloud('classes', demoClasses);
      await deleteCloud('students', demoStudents);
      await deleteCloud('class_students', demoClassStudents);
      await deleteCloud('sessions', demoSessions);
      await deleteCloud('student_sessions', demoStudentSessions);
      await deleteCloud('warnings', demoWarnings);
      await deleteCloud('ai_diagnoses', demoAiDiagnoses);
      await deleteCloud('knowledge_results', demoKnowledgeResults);
      await deleteCloud('knowledge_tags', demoTags);
      await deleteCloud('audit_logs', demoAuditLogs);
      await deleteCloud('school_years', demoSchoolYears);
    } catch (err) {
      console.warn(`[Delete Demo] Failed to sync delete to Firestore:`, err);
    }
  }
}
...
export async function clearAllDataToBlankSlate() {
  ...
  // Ensure demo data seed is disabled
  localStorage.setItem('seed_demo_data_done', 'true');

  // Turn off Cloud Sync so that the old data is not pulled back immediately
  localStorage.setItem('cloud_sync_enabled', 'false');

  // Try to delete documents from Firestore collections to clean up the cloud
  try {
    if (firestoreDb) {
      ...
      for (const colName of collectionsToClear) {
        try {
          ...
          console.warn(`[Blank Slate] Cloud clear error for ${colName} (likely due to quota limit):`, colErr);
        }
      }
    }
  } catch (err) {
    console.error('[Blank Slate] Failed to clear Cloud Firestore collections:', err);
  }
}
