...
export const StudentRoster: React.FC<StudentRosterProps> = ({
  ...
  const handleSaveStudent = async (e: React.FormEvent) => {
    ...
    if (editingStudent) {
      ...
      if (firestoreDb) {
        await safeSetDoc(doc(firestoreDb, 'students', String(editingStudent.id!)), {
          ...editingStudent,
          ...updatePayload,
      ...
      if (firestoreDb) {
        await safeSetDoc(doc(firestoreDb, 'students', String(studentId)), {
          ...addPayload,
          id: studentId
        });
      }
      ...
      for (const classId of enrollClassIds) {
        ...
        const csId = await db.class_students.add(csPayload);
        if (firestoreDb) {
          await safeSetDoc(doc(firestoreDb, 'class_students', String(csId)), {
            ...csPayload,
            id: csId
          });
        }
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/60">
                  <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 mb-3 flex items-center gap-1.5">
                    ...
                    <span>Biểu Đồ Lịch Sử Điểm BTVN & Bài Kiểm Tra Qua Các Buổi</span>
                  </h4>

                  <div className="h-56">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={studentPerformanceHistory} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        />
                        <Line type="monotone" dataKey="hwScore" name="Điểm BTVN" stroke="#10b981" strokeWidth={2.5} activeDot={{ r: 6 }} />
                        <Line type="monotone" dataKey="testScore" name="Điểm Kiểm Tra" stroke="#0284c7" strokeWidth={2.5} activeDot={{ r: 6 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>

                {/* Target Progress Benchmark Card */}
                {(() => {
                  const validTest = studentPerformanceHistory.filter(h => h.testScore !== null && h.testScore !== undefined);
                  const avgTest = validTest.length === 0 ? 0 : validTest.reduce((sum, h) => sum + (h.testScore || 0), 0) / validTest.length;
                  
                  // Extract active class and its target description
                  ...
                  const targetDesc = activeClassObj?.target_description || 'Thi vào Lớp 10 Công lập 8.0+';
                  
                  // Parse target score value
                  const match = targetDesc.match(/(\d+(\.\d+)?)/);
                  const targetVal = match ? parseFloat(match[1]) : 8.0;

                  const gap = avgTest - targetVal;
                  const percentVal = Math.min(100, Math.max(0, Math.round((avgTest / targetVal) * 100)));
                  ...
                  if (avgTest === 0) {
                    statusText = 'Chưa có đủ điểm kiểm tra';
                    statusColor = 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400';
                    progressColor = 'bg-slate-300';
                  } else if (gap >= 0) {
                    statusText = `Vượt mục tiêu (+${gap.toFixed(1)}đ)`;
                    statusColor = 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 border border-emerald-200/50';
                    progressColor = 'bg-emerald-500';
                  } else if (gap >= -1.0) {
                    statusText = `Tiệm cận mục tiêu (thiếu ${Math.abs(gap).toFixed(1)}đ)`;
                    statusColor = 'bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400 border border-amber-200/50';
                    progressColor = 'bg-amber-500';
                  } else {
                    statusText = `Cần bồi dưỡng khẩn cấp (thiếu ${Math.abs(gap).toFixed(1)}đ)`;
                    statusColor = 'bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-400 border border-rose-200/50';
                    ...
                    <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
                      <div className="flex items-center justify-between">
                        <div className="space-y-0.5">
                          <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block">Mục tiêu lớp học:</span>
                          <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{targetDesc}</span>
                        </div>
                        <span className={`px-2 py-1 text-[10px] font-extrabold rounded-lg ${statusColor}`}>
                          {statusText}
                        </span>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-[11px] font-bold text-slate-500">
                          ...
                          <span className="text-slate-800 dark:text-slate-200">{percentVal}%</span>
                        </div>
                        <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2.5 overflow-hidden">
                          <div className={`h-full rounded-full ${progressColor} transition-all duration-500`} style={{ width: `${percentVal}%` }} />
                        </div>
                        <div className="flex justify-between items-center pt-1 text-[10px] text-slate-400">
                          <span>Điểm trung bình hiện tại: <strong className="text-slate-700 dark:text-slate-300 font-bold">{avgTest > 0 ? avgTest.toFixed(1) : 'N/A'}đ</strong></span>
                          <span>Điểm chuẩn mục tiêu: <strong className="text-slate-700 dark:text-slate-300 font-bold">{targetVal.toFixed(1)}đ</strong></span>
                        </div>
                      </div>
                    </div>
                  );
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-slate-50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800/60 text-center">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase">Điểm TB BTVN</span>
                    <span className="text-lg font-extrabold text-emerald-600 dark:text-emerald-400">
                      {(() => {
                        const valid = studentPerformanceHistory.filter(h => h.hwScore !== null && h.hwScore !== undefined);
                        if (valid.length === 0) return 'N/A';
                        const avg = valid.reduce((sum, h) => sum + h.hwScore, 0) / valid.length;
                        return avg.toFixed(1);
                      })()}
                    </span>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800/60 text-center">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase">Điểm TB Kiểm Tra</span>
                    <span className="text-lg font-extrabold text-sky-600 dark:text-sky-400">
                      {(() => {
                        const valid = studentPerformanceHistory.filter(h => h.testScore !== null && h.testScore !== undefined);
                        if (valid.length === 0) return 'N/A';
                        const avg = valid.reduce((sum, h) => sum + h.testScore, 0) / valid.length;
                        return avg.toFixed(1);
                      })()}
                    </span>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800/60 text-center">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase">Tỷ Lệ Có Mặt</span>
                ...
                <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/60 space-y-3">
                  ...
                  <div className="space-y-2.5">
                    {(() => {
                      ...
                      return studentMemberships.map((cs) => {
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2.5">
                  ...
                  {studentWarnings.filter(w => !w.resolved).length === 0 ? (
                    ...
                    studentWarnings.filter(w => !w.resolved).map((w) => {
