...
export const UltraFastGradeEntry: React.FC<UltraFastGradeEntryProps> = ({
  ...
  // Active view tab: 'entry' or 'knowledge_map'
  const [activeTab, setActiveTab] = useState<'entry' | 'knowledge_map'>('entry');
  ...
  const handleCreateNewSession = async (e: React.FormEvent) => {
    ...
    if (firestoreDb) {
      await safeSetDoc(doc(firestoreDb, 'sessions', String(newId)), {
        ...sessionPayload,
        id: newId
      });
    }
  ...
  const handleSaveEditedSession = async (e: React.FormEvent) => {
    ...
    if (firestoreDb) {
      await safeSetDoc(
        doc(firestoreDb, 'sessions', String(selectedSessionId)),
        {
          ...selectedSess,
          session_date: editSessionDate,
          lesson_title: fullLessonTitle,
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                ...
                <span>Nhập Điểm Tốc Độ Cao (Ultra-Fast Grade Entry)</span>
              </h2>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Hỗ trợ phím Tab, Enter, Mũi tên nhập điểm nhanh cho 30+ học sinh & Tích hợp bộ nhận xét Toán THCS.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="px-3 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl text-xs font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-all flex items-center gap-1.5"
            >
            ...
            <button
              ...
              disabled={!selectedSessionId}
              className="px-3 py-2 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 rounded-xl text-xs font-bold hover:bg-emerald-100 transition-all flex items-center gap-1.5 shadow-sm disabled:opacity-50"
            >
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
        ...
        {saveSuccessMsg && (
          <div className="px-3 py-1.5 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-200 border border-emerald-200 dark:border-emerald-800/80 rounded-xl text-xs font-bold flex items-center gap-1.5 animate-bounce">
            ...
            <span>Đã lưu thành công dữ liệu lên đám mây Firestore, tính năng hoàn thành!</span>
          </div>
        )}
      </div>
              ...
              <table className="w-full text-left border-separate border-spacing-0">
                <thead className="sticky top-0 z-20 shadow-xs">
                  <tr className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[11px] uppercase tracking-wider font-extrabold">
                    ...
                    <th className="py-3 px-4 min-w-[280px] sticky top-0 z-20 bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">Nhận Xét & Preset Toán THCS</th>
                  </tr>
                </thead>
