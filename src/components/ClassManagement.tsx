...
export const ClassManagement: React.FC<ClassManagementProps> = ({
  ...
  const handleSaveClass = async (e: React.FormEvent) => {
    ...
    if (editingClass) {
      ...
      if (firestoreDb) {
        await safeSetDoc(doc(firestoreDb, 'classes', String(editingClass.id!)), {
          ...editingClass,
          ...updatePayload,
          id: editingClass.id
        }, { merge: true });
      ...
      const newClassId = await db.classes.add(addPayload);
      if (firestoreDb) {
        await safeSetDoc(doc(firestoreDb, 'classes', String(newClassId)), {
          ...addPayload,
          id: newClassId
        });
      }
  ...
  const handleArchiveClass = async (cls: ClassItem) => {
    ...
    if (firestoreDb) {
      await safeSetDoc(doc(firestoreDb, 'classes', String(cls.id!)), {
        ...cls,
        status: newStatus,
        updated_at: now,
        id: cls.id
  ...
  const handleExecutePromote = async () => {
    ...
    if (firestoreDb) {
      ...
      await safeSetDoc(doc(firestoreDb, 'classes', String(promotingClass.id!)), {
        ...promotingClass,
        status: 'archived',
        updated_at: now,
        id: promotingClass.id
      }, { merge: true });
    }
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-slate-700 dark:text-slate-300">
                      {(() => {
                        ...
                        return classStudLinks.map((link, idx) => {
                          ...
                          return (
                            <tr key={link.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/30 ${!isCurrentlyActive ? 'opacity-60 bg-slate-50/40 dark:bg-slate-900/40' : ''}`}>
