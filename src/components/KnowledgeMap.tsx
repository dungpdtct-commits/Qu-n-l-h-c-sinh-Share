...
export const KnowledgeMap: React.FC<KnowledgeMapProps> = ({
  ...
  const handleSaveTag = async () => {
    ...
    if (editingTag) {
      ...
      if (firestoreDb) {
        await safeSetDoc(doc(firestoreDb, 'knowledge_tags', String(editingTag.id!)), {
          ...editingTag,
          ...updatePayload,
          id: editingTag.id
        }, { merge: true });
      ...
      const newTagId = await db.knowledge_tags.add(addPayload);
      if (firestoreDb) {
        await safeSetDoc(doc(firestoreDb, 'knowledge_tags', String(newTagId)), {
          ...addPayload,
          id: newTagId
        });
      }
  ...
  const handleDeleteTag = async (tag: KnowledgeTag) => {
    if (
      ...
      if (firestoreDb) {
        await safeDeleteDoc(doc(firestoreDb, 'knowledge_tags', String(tag.id!)));
      }
