...
export const WarningCenter: React.FC<WarningCenterProps> = ({
  ...
  const [priorityFilter, setPriorityFilter] = useState<'P1' | 'P2' | 'all'>('all');
...
Cảm ơn sự đồng hành sát sao từ quý gia đình trong thời gian qua để giúp con tiến bộ hơn mỗi ngày!`;
  ...
  const handleExecuteResolve = async () => {
    ...
    if (firestoreDb && resolvingWarning.id) {
      await safeSetDoc(doc(firestoreDb, 'warnings', String(resolvingWarning.id)), {
        ...resolvingWarning,
        resolved: true,
        resolved_action: fullActionDesc,
        updated_at: now,
        id: resolvingWarning.id
      }, { merge: true });
    }
        ...
        <>
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-slate-900 dark:text-slate-100">
                  Hệ Thống Cảnh Báo Thông Minh (Smart Warning Rules Engine)
                </h2>
              </div>
              <p className="text-xs text-slate-500 mt-1">
                Phân tích tự động dựa trên dữ liệu thời gian thực: Điểm thi &lt; {ruleConfig.minTestScore}đ ({ruleConfig.consecutiveLowTests} buổi liền), Thiếu BTVN ({ruleConfig.consecutiveLowHomework} buổi), Vắng không phép (&gt;= {ruleConfig.maxAbsences} buổi).
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                disabled={isScanning}
                className="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-600/20 flex items-center gap-1.5 disabled:opacity-50"
                title="Khởi tạo dữ liệu điểm & các kịch bản kiểm thử cảnh báo học tập"
              >
                <Zap className="w-4 h-4 text-amber-300 fill-amber-300" />
                <span>⚡ Nạp Kịch Bản Kiểm Thử</span>
              </button>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-200 dark:border-slate-800">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              >
                <option value="all">Tất cả Cấp Độ (P1, P2 & Praise)</option>
                <option value="P1">P1 - Khẩn cấp (Màu đỏ)</option>
                <option value="P2">P2 - Nội bộ (Màu vàng)</option>
                ...
                <div className="bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800/60 p-3.5 rounded-xl space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    ...
                    <div>
                      ...
                      >
                        {cycleSessions.map((s, idx) => (
                          <option key={s.id} value={idx} disabled={idx < cycleStartIdx}>
                            Buổi {idx + 1} ({(s?.session_date || '').slice(5)}) - {(s?.lesson_title || '').slice(0, 15)}...
                          </option>
                        ))}
                      </select>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800 p-4 space-y-3">
                ...
                {studentActiveWarnings.length === 0 ? (
                  ...
                  <div className="space-y-2.5 max-h-[180px] overflow-y-auto">
                    {studentActiveWarnings.map((w) => (
                      <div key={w.id} className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex items-center justify-between gap-3 text-[11px] shadow-sm">
